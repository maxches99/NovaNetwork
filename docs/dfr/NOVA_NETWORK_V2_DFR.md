...
| NFR-4 | Determinism | Unit tests use injected transports/clocks and no external network; telemetry recorders preserve synchronous emission order without unstructured per-event tasks; scheduler-sensitive timeout tests establish that the operation is in flight before measuring elapsed time. | T-GATE-3, T-GATE-4, test policy audit |
...
| NFR-4 | T-GATE-4 `inFlightTimeoutEvictsHungEntries` synchronizes on an observable in-flight entry before starting the timeout window; shared telemetry hooks record synchronously through a lock-backed recorder | unit/integration | Engineering | passed (timeout 100/100; policy telemetry 50/50; full suite 3/3 on local Swift 6.3) |
...
- Combined source line coverage: 92.45% across `NovaNetworkClient` and `NovaNetworkCore`.
